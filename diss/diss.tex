...
The average single task time is 4.13 ms.
...
\item \textbf{Should} write data quickly. It should take under 5 ms to write a short string to its persistent store. It was calculated in the same way as the execution data, using the following code, which writes ``Hello, world" into JSON keys ``0", ``1", ``2" ... ``99".
...
The average single write time is 2.49 ms.
...
\item \textbf{Should} be low-latency, sending a message and receiving a reply quickly. The following code was put on two devices, one of which acted as a server and the other, the client. The client queues RPC on the server, and when it executes successfully, the server queues successs("finished") on the client, which is then printed out. This test is repeated 10 times.
...
The average of the single task time is 4.13 ms, beating the minimum requirement by almost 20\%. The times are also very close to each other, meaning that there is a high degree of certainty about how long preparing to execute a function of constant size will take.
